...
class MoviePlanTests(unittest.TestCase):
    ...
    def test_build_movie_plan_detects_verbose_folder_after_concise_file_rename(self) -> None:
        folder = self.source / "The Matrix (1999) [1080p BluRay x264 AAC GRP]"
        folder.mkdir()
    ...
    def test_build_movie_plan_rescan_is_clean_after_concise_apply(self) -> None:
        ...
        rescanned = build_movie_plan(self.source)

        self.assertEqual(len(report.applied), 2)
        self.assertEqual(rescanned.proposed_changes, [])
        self.assertTrue((self.source / "The Matrix (1999)" / "The Matrix (1999).mkv").exists())

    def test_build_movie_plan_marks_existing_concise_target_collision_for_review(self) -> None:
        concise = self.source / "Ace Ventura Pet Detective (1994)"
        verbose = self.source / "Ace Ventura Pet Detective (1994) [BluRay 1080p DDP 5.1 x264 HALLOWED]"
        concise.mkdir()
        verbose.mkdir()
    ...
    def test_build_movie_plan_uses_differentiator_for_package_split_existing_target_collision(self) -> None:
        canonical = self.source / "Ace Ventura When Nature Calls (1995)"
        canonical.mkdir()
        (canonical / "Ace Ventura When Nature Calls (1995).mkv").write_text("video", encoding="utf-8")
        collection = self.source / "The Ace Ventura Collection (1994-1995)"
        collection.mkdir()
        (
    ...
    def test_build_movie_plan_uses_differentiator_for_folder_existing_target_collision(self) -> None:
        canonical = self.source / "Battle Royale (2000)"
        canonical.mkdir()
        (canonical / "Battle Royale (2000).mkv").write_text("video", encoding="utf-8")
        verbose = self.source / "Battle.Royale.2000.1080p.BluRay.3D.H-SBS.DTS.x264-PublicHD"
        verbose.mkdir()
        (
    ...
    def test_build_movie_plan_deletes_no_video_movie_artifact_folder_with_only_metadata(self) -> None:
        artifact = self.source / "A Few Good Men (1992) [1080p BluRay AC3 x264 ETRG]"
        artifact.mkdir()
    ...
    def test_build_movie_plan_deletes_metadata_only_collection_artifact_folder(self) -> None:
        artifact = self.source / "The Mummy 1, 2, 3, 4 - Collection 1999-2017 Eng Subs 1080p [H264-mp4]"
    ...
    def test_build_movie_plan_deletes_empty_no_video_folder(self) -> None:
        empty = self.source / "Lonely Empty Folder"
    ...
    def test_build_movie_plan_deletes_no_video_folder_with_promo_document_spam(self) -> None:
        residue = self.source / "Movie Spam Folder"
    ...
    def test_build_movie_plan_keeps_no_video_folder_with_neutral_text_note(self) -> None:
        residue = self.source / "Movie Notes Folder"
        residue.mkdir()
    ...
    def test_build_movie_plan_moves_loose_movie_with_sidecar_nfo_identity(self) -> None:
        movie = self.source / "Se7en [1080p, x264, dts ita, ac3 ita-eng, subs] by GuglielmoDaBaskerville.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_deletes_root_appledouble_junk_files(self) -> None:
        (self.source / "._Movie.mkv").write_text("metadata", encoding="utf-8")
    ...
    def test_build_movie_plan_normalizes_multi_part_movie_folder(self) -> None:
        folder = self.source / "White Mischief (1987) Greta Scacchi 2CD 2160p H.264 ENG-GER (moviesbyrizzo) MULTISUB"
        folder.mkdir()
    ...
    def test_build_movie_plan_deletes_metadata_and_poster_residue_when_winner_exists(self) -> None:
        concise = self.source / "A Few Good Men (1992)"
        artifact = self.source / "A Few Good Men (1992) [1080p BluRay AC3 x264 ETRG]"
        concise.mkdir()
        artifact.mkdir()
    ...
    def test_build_movie_plan_splits_technical_tokens_before_trailing_year(self) -> None:
        (self.source / "Hooligans Bdrip 1080p Ita Eng X264 Blauray (2005).mkv").write_text("video", encoding="utf-8")
        (self.source / "Spaceballs Bdrip 1080p ENG ITA FRE GER SPA Multisub X264 Bluray (1987).mkv").write_text(
            "video",
            encoding="utf-8",
        )
    ...
    def test_build_movie_plan_moves_loose_root_movies_into_concise_folders_by_default(self) -> None:
        (self.source / "Dune.1984.Extended.1080p.BluRay.AC3.x264-ETRG.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_adds_concise_resolution_differentiators_for_duplicate_titles(self) -> None:
        first = self.source / "Movie.One"
        second = self.source / "Movie.Two"
        first.mkdir()
        second.mkdir()
    ...
    def test_build_movie_plan_keeps_root_level_duplicate_titles_as_review_without_local_context(self) -> None:
        normalized = self.source / "Collision Movie (2001)"
        normalized.mkdir()
        (normalized / "Collision Movie (2001).mkv").write_text("video", encoding="utf-8")
        (self.source / "Collision.Movie.2001.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_marks_composed_file_target_collision_for_review(self) -> None:
        verbose = self.source / "Ace Ventura Pet Detective (1994) 1080p"
        verbose.mkdir()
    ...
    def test_build_movie_plan_handles_dotted_edition_resolution_and_release_group(self) -> None:
        folder = self.source / "Dune Folder"
        folder.mkdir()
    ...
    def test_build_movie_plan_keeps_known_structure_safe_despite_long_unknown_tail_token(self) -> None:
        movie = self.source / "The Last Starfighter (1984) 25th Anniversary Ed 1080p Blu-ray x264 DTS-HDMA 5.1-DTOne.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_still_reviews_long_unknown_tail_token_when_structure_is_weak(self) -> None:
        movie = self.source / "Mystery Film (1984) AnniversaryWord.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_splits_bluray_remux_and_preserves_language_tags(self) -> None:
        movie = self.source / "Land.of.the.Dead.2005.BluRayRemux.1080p.x264.3Rus.Eng.-CME-v0.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_cleans_dotted_collection_parent(self) -> None:
        collection = self.source / "The.Godfather.Trilogy.[ I. II. III ].1080p.BluRay.x264.anoXmous"
    ...
    def test_build_movie_plan_collapses_duplicate_single_movie_wrapper(self) -> None:
        wrapper = self.source / "A.Dangerous.Method.2011.1080p.MKV.x264.AC3.DTS.MultiSubs"
    ...
    def test_build_movie_plan_moves_loose_root_movies_into_canonical_folders(self) -> None:
        (self.source / "Dune.1984.Extended.1080p.BluRay.AC3.x264-ETRG.mkv").write_text("video", encoding="utf-8")
        (self.source / "Zootopia.2016.2160p.uhd.bluray.x265-terminal.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_preserves_known_technical_punctuation(self) -> None:
        movie = self.source / "John.Wick.Chapter.2.2017..Blu-Ray.1080p.HDR.HEVC.DD.5.1-DDR.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_reconstructs_k19_punctuation_for_raw_file(self) -> None:
        movie = self.source / "K.19.The.Widowmaker.2002.1080p.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_reconstructs_compact_k19_punctuation_for_raw_file(self) -> None:
        movie = self.source / "K19.The.Widowmaker.2002.1080p.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_reconstructs_ordinal_suffixes_for_raw_files(self) -> None:
        (self.source / "25Th.Hour.2002.1080p.mkv").write_text("video", encoding="utf-8")
        (self.source / "Friday.The.13Th.1980.1080p.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_reconstructs_abbreviation_punctuation_for_raw_files(self) -> None:
        (self.source / "L.A.Confidential.1997.1080p.mkv").write_text("video", encoding="utf-8")
        (self.source / "Mr.Nobody.2009.1080p.mkv").write_text("video", encoding="utf-8")
        (self.source / "Dr.Strangelove.1964.1080p.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_marks_safe_legacy_normalized_punctuation_upgrade(self) -> None:
        folder = self.source / "K 19 The Widowmaker (2002)"
        folder.mkdir()
    ...
    def test_build_movie_plan_marks_safe_legacy_normalized_ordinal_and_abbreviation_upgrades(self) -> None:
        ordinal_folder = self.source / "25Th Hour (2002)"
        ordinal_folder.mkdir()
        (ordinal_folder / "25Th Hour (2002).mkv").write_text("video", encoding="utf-8")
        abbreviation_folder = self.source / "Mr Nobody (2009)"
    ...
    def test_build_movie_plan_drops_noop_collision_resolution_changes(self) -> None:
        canonical = self.source / "Legend (1985)"
        canonical.mkdir()
        (canonical / "Legend (1985).mkv").write_text("video", encoding="utf-8")

        alternate = self.source / "Legend (1985) 1080p"
        alternate.mkdir()
    ...
    def test_build_movie_plan_promotes_known_canonical_punctuation_upgrades_to_safe(self) -> None:
        ...
        for current_base, expected_base in cases.items():
            folder = self.source / current_base
            folder.mkdir()
    ...
    def test_build_movie_plan_does_not_overreach_into_stable_numeric_title_punctuation(self) -> None:
        folder = self.source / "THX 1138 (1971)"
        folder.mkdir()
    ...
    def test_build_movie_plan_does_not_overreach_into_unsettled_punctuation(self) -> None:
        (self.source / "21.Jump.Street.2012.1080p.mkv").write_text("video", encoding="utf-8")
        (self.source / "Mission.Impossible.Dead.Reckoning.Part.One.2023.1080p.mkv").write_text("video", encoding="utf-8")
        (self.source / "The.Devils.Advocate.1997.1080p.mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_splits_leading_number_compact_bracket_payload(self) -> None:
        folder = self.source / "(2001) [ASPACEODYSSEY1968V22160PBLURAYX26510BITHDRTIGOLE]"
        folder.mkdir()
    ...
    def test_build_movie_plan_prefers_parenthesized_year_after_numeric_title(self) -> None:
        folder = self.source / "2001 - A Space Odyssey (1968) V2 (2160p BluRay x265 HEVC 10bit HDR AAC 5.1 Tigole)"
        folder.mkdir()
    ...
    def test_build_movie_plan_handles_parenthesized_numeric_title_with_bracketed_year(self) -> None:
        folder = self.source / "(1917) [2019 1080p BluRay Atmos TrueHD 7.1 x264 EVO]"
        folder.mkdir()
    ...
    def test_build_movie_plan_handles_dotted_numeric_title_with_release_year(self) -> None:
        movie = self.source / "1917.2019.1080p.Bluray.Atmos.TrueHD.7.1.x264-EVO.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_treats_common_long_descriptors_as_safe(self) -> None:
        movie = self.source / "A.Nightmare.on.Elm.Street.1984.Remastered.1080p.BluRay.x265.hevc.10bit.AAC.7.1.commentary-HeVK.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_strips_leading_website_credit(self) -> None:
        movie = self.source / "www.UIndex.org    -    Wings Of Desire 1987 1080p MAX WEB-DL DDP5 1 H 264-GPRS.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_strips_spaced_website_credit(self) -> None:
        movie = self.source / "Www Hdsector Com Hachi A Dog's Tale (2009) [BluRay 1080p x264 AAC 5.1 HON3Y].mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_uses_parent_year_with_yearless_release_filename(self) -> None:
        folder = self.source / "I Want to Eat Your Pancreas (2018)"
        folder.mkdir()
    ...
    def test_build_movie_plan_uses_known_yearless_title_hint_for_pancreas_release(self) -> None:
        movie = self.source / "[Arid] I Want to Eat Your Pancreas (BDRip 1080p HEVC OPUS) [BCB2E441].mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_strips_generic_leading_bracket_credit(self) -> None:
        folder = self.source / "The Apartment (1960)"
        folder.mkdir()
    ...
    def test_build_movie_plan_allows_nine_character_unknown_tokens(self) -> None:
        movie = self.source / "Boundary.1999.NineChars.1080p.mkv"
        movie.write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_splits_compact_technical_tokens(self) -> None:
        folder = self.source / "Basic Instinct (1992) [Unrated 1080PENGITAMULTISUBX264BLURAYSHIV]"
        folder.mkdir()
    ...
    def test_build_movie_plan_uses_resolution_differentiator_for_shining_duplicate(self) -> None:
        clean = self.source / "The Shining (1980)"
        noisy = self.source / "The Shining (1980) [Extended 1072p BluRay 5.1 x264 NVEE]"
        clean.mkdir()
        noisy.mkdir()
        ...
        self.assertTrue((self.source / "The Shining (1980) 1072p" / "The Shining (1980) 1072p.mkv").exists())
    ...
    def test_build_movie_plan_merges_subtitle_only_residue_into_existing_winner(self) -> None:
        clean = self.source / "The Shining (1980)"
        residue = self.source / "The Shining (1980) [1080p BluRay x264]"
        clean.mkdir()
        residue.mkdir()
        (clean / "The Shining (1980).mkv").write_text("video", encoding="utf-8")
    ...
    def test_build_movie_plan_deletes_empty_package_artifacts_without_year(self) -> None:
        godfather = self.source / "The Godfather Trilogy [I II III] 1080p BluRay x264 anoXmous"
        apes = self.source / "Planet of the Apes Legends The Collection 1080p H 264 ENG-FRE-SPA (moviesbyrizzo)"
        godfather.mkdir()
        apes.mkdir()
    ...
    def test_build_movie_plan_splits_distinct_movies_from_package_folder(self) -> None:
        folder = self.source / "Short Circuit Duology (1986-1988) BDRip 1080p HighCode"
        folder.mkdir()
    ...
    def test_build_movie_plan_does_not_project_trilogy_marker_into_year_only_package_files(self) -> None:
        folder = self.source / "Mad Max Trilogy 1080p BluRay x264"
        folder.mkdir()
        for filename in [
    ...
    def test_build_movie_plan_splits_ampersand_double_feature_when_files_parse(self) -> None:
        folder = self.source / "Grindhouse Planet Terror & Death Proof 1080p Unrated [mkvonly]"
        folder.mkdir()
    ...
    def test_build_movie_plan_keeps_package_folder_when_existing_target_collision_needs_salvage(self) -> None:
        canonical = self.source / "Death Proof (2007)"
        canonical.mkdir()
    ...
    def test_build_movie_plan_trims_reported_verbose_parser_slips(self) -> None:
        for filename in [
            ...
            (self.source / filename).write_text("video", encoding="utf-8")
